	...
	if (file_exists($log_file)) {
		...
		if (date('j') == 10) {
			...
			if (!file_exists($prev_name)) {
				...
				if ($type == 0) {
					$toWrite = "1";
					$visitor_count = $beforeAllText . "1";
				} else if ($type == 1) {
					$toWrite = "1;" . $uIP . ",";
					$visitor_count = $beforeUniqueText . "1";
				} else if ($type == 2) {
					$toWrite = "1;1;" . $uIP . ",";
					$visitor_count = $beforeAllText . "1" . $separator . $beforeUniqueText . "1";
				}
				goto write_logfile;
			}
		}
		...
		if ($type == 0) {
			...
			$visitor_count = $beforeAllText . $toWrite;
			...
			//Info to show.
			$visitor_count = $beforeUniqueText . $hits;
			...
			//Info to show.
			$visitor_count = $beforeAllText . $allHits . $separator . $beforeUniqueText . $uniqueHits;
		}
	} else {
