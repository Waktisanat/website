...
foreach ($crafts as $craftJSON) {
    $craft = json_decode( $craftJSON );
    $item = new Item( $craft->Item );
    $myRecette = Recette::get_recettes_by_item_id($item);
    $ingredients = $myRecette[0]->load_direct_ingredients();

    $return .= '<li>';
    $return .= '<fieldset class="'.$craft->Item.'" style="position: relative;">';
    $return .= '<a data-item="'.$craft->Item.'" class="red delete" title="Supprimer de ma liste de craft" style="position: absolute; right: -0.80rem; top: -0.35rem;"><i class="fa fa-times fa-2x"></i></a>';
